...
```sh
go run github.com/jfrog/jfrog-cli-pipelines command [options] [arguments...]
```
...
```
go test -v github.com/jfrog/jfrog-cli-pipelines [test-types] [flags]
```
...
| `-jfrog.sshKeyPath`    | [Optional] Path to the SSH key file. Use this flag only if the Pipelines URL format is `ssh://[domain]:port`. |
...
- Before running the tests, wait for Pipelines to finish booting up in the container.

| Flag                      | Description                         |
| ------------------------- | ----------------------------------- |
| `-test.containerRegistry` | Pipelines Docker registry domain. |
